...
$(document).ready(function () {
    ...
    item_remove.on('click', function () {
        var rowCount = $('#invoice_items tr').length;
        if (rowCount > 2) {
            $invoice_table.find('tr:last').fadeOut(300, function () {
                $(this).remove();
                calculateTotal();
            });
            var rowCount = $('#invoice_items tr').length;
    ...
    blank_add.on('click', function () { 
        $invoice_table.find('tbody').append(
            ...
            '<td data-label="Price"><input type="number" class="form-control item_price" name="amount[]" value="" autocomplete="off"></td>'+
            '<td data-label="Qty"><input type="number" class="form-control qty" value="" name="qty[]" autocomplete="off" ></td>'+
            '<td data-label="Tax"><input type="number" class="form-control tax" name="taxed[]" value="" max="99"> </td>'+
            '<td data-label="Discount"><input type="number" class="form-control discount" name="discount[]" value="" min="0" max="99"> </td>'+
